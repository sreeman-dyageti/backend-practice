...
app.get("/secrets", async(req, res) => {
  if (req.isAuthenticated()) {
    try {
      const result = await db.query("SELECT secret FROM register WHERE email = $1", [req.user.email]);
...
app.post("/submit", async (req,res)=>{
  ...
  try {
    await db.query("UPDATE register SET secret = $1 WHERE email = $2",[secret, req.user.email]);
  } catch (error) {
    console.log(error);
  }
  res.redirect("/secrets");
